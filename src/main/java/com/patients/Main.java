...
public class Main {
    ...
    // Default location of static web resources (HTML/CSS/JS) relative to the
    // project root.
    ...
    // Mapping our build output here lets Tomcat load our servlets without needing a
    // packaged WAR.
    ...
    private static int getPort() {
        // System properties are usually set via JVM arguments; environment variables
        // come from the OS.
        String port = System.getProperty("SERVER_PORT", System.getenv("SERVER_PORT"));
        return (port != null) ? Integer.parseInt(port) : DEFAULT_PORT;
    }

    // Read the webapp directory (static resources) from a system property or
    // environment variable.
    // This allows the same code to run in different environments without
    // recompiling.
    private static String getWebappDir() {
        // Prefer system property; if missing, try environment variable; otherwise
        // default.
        String dir = System.getProperty("WEBAPP_DIR", System.getenv("WEBAPP_DIR"));
        return (dir != null) ? dir : DEFAULT_WEBAPP_DIR;
    }

    // Read the directory containing compiled .class files from a system property or
    // environment variable.
    ...
    // A shutdown hook runs when the JVM is exiting (e.g., Ctrl+C, SIGTERM, IDE stop
    // button).
    // We use it to stop and destroy Tomcat so resources (threads, sockets) are
    // released cleanly.
    public static Thread addShutdown(final Tomcat tomcat, final Logger logger) {
        // The shutdown hook is a Thread. The code in the lambda runs when the JVM
        // begins shutdown.
        Thread shutdownHook = new Thread(() -> {
            try {
                // Defensive check: avoid NullPointerException if startup failed before Tomcat
                // was created.
                if (tomcat != null) {
    ...
    private static Logger initialiseLogger() {
        ...
        // ConsoleHandler prints log records to standard error (typically shown in the
        // IDE/terminal).
        ConsoleHandler consoleHandler = new ConsoleHandler();
        consoleHandler.setLevel(Level.INFO);
        logger.addHandler(consoleHandler);

        // FileHandler writes log records to a file. Creating it can fail (e.g.,
        // permissions),
        // so we handle IOException.
        try {
            // Create/append a log file. By default FileHandler may rotate; here we use a
            // simple single file.
            FileHandler fileHandler = new FileHandler(LOGFILE);
            // SimpleFormatter produces human-readable log lines (timestamp, level, message,
            // etc.).
    ...
    // We add the compiled classes directory as a "pre-resource" mapped into
    // /WEB-INF/classes.
    ...
    private static void setResources(Context context, Path targetClassesDirectory) {
        ...
        // We map the build output directory into /WEB-INF/classes so Tomcat can load
        // servlets/classes.
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // Without this, some embedded setups don't fully initialise the connector until
            // start().
            tomcat.getConnector();
            ...
            // Make Tomcat's servlet API (and other Maven deps) visible to the webapp
            // classloader by delegating to the exec:java thread classloader.
        ...
        // Catch-all for unexpected startup errors. In production code you might handle
        // specific exceptions.
        catch (Exception e) {
